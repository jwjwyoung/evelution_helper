...
def extract_queries_and_scopes(app_dir, output_dir, rails_best_practices_cmd, version=nil)
    ...
    RawQuery.new(canonicalize_classname(obj[:class]), obj[:stmt], false, 
              obj[:caller_class_lst].map! {|x|
                                  x[:class_name]=canonicalize_classname(x[:class_name].to_s)
                                          x }.to_a, 
                  obj[:method_name], obj[:filename], obj[:line]) 
  end
  ...
  handle_fields = FALSE
  if version and not File.exist?(app_dir + "/db/schema.rb")
    handle_fields = TRUE
  end
  schema = Marshal.load(File.binread(schema_output_file)).map do |klass, hmap|
    ...
    if handle_fields
      fields = {} 
      puts "class name #{klass.class_name}"
      if version.activerecord_files[klass.class_name]
        version.activerecord_files[klass.class_name].columns.map{|c, v| fields[v.column_name] = v.column_type}
...
def load_constraints(app_dir, output_dir, constraint_analyzer_dir)
  ...
  r = Marshal.load(File.binread(output_file)).map do |obj|
    Constr.new(obj[:table], obj[:type], obj[:fields], obj[:exists_in_db], nil) 
  end
...
def load_extra_constraints(app_name)
  ...
  File.open(constraint_file).read.each_line do |line|
    ...
    if words[1].nil?
      ...
      r << Constr.new(words[0], :inclusion, [words[1]], false, '')
    end
  end
	...
	extra_constraints.each do |constraint|
		r << Constr.new(constraint["table"], constraint["type"], constraint["fields"], false, constraint["source_code"])
	end
	r
end


def load_queries_and_schema(app_dir, output_dir, rails_best_practices_cmd, version=nil)
  queries,scopes,schema = extract_queries_and_scopes(app_dir, output_dir, rails_best_practices_cmd, version)
